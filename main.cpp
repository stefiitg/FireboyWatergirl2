...
public:
    ...
    Game(int mapW = 14, int mapH = 9)
        : window(sf::VideoMode( (unsigned)(mapW * Tile::getSize()), (unsigned)(mapH * Tile::getSize()) ), "Fireboy & Watergirl"),
          map(mapW, mapH),
          fireboy("Fireboy", "assets/fireboy.png", {Tile::getSize()*1.f, Tile::getSize()*(mapH-2.f)}, 3, sf::Color::Red),
          watergirl("Watergirl", "assets/watergirl.png", {Tile::getSize()*5.f, Tile::getSize()*(mapH-2.f)}, 3, sf::Color::Blue)
    {
        // generate platforms
        map.generateCustomPlatforms();

        // try load font
        if (!font.loadFromFile("assets/arial.ttf")) {
            // fallback: use system default? SFML needs a file; if not found, we still proceed but win text won't show well.
            cout << "Warning: font 'assets/arial.ttf' not found. Win text may not display correctly.\n";
        }
